        ...
        NavigationView {
            ZStack {
                ...
                VStack {
                    ...
                    ToolbarModelView(title: "Главная панель", hasBackButt: false, button1Icon: IMAGE_OPTION_ICON, button2Icon: IMAGE_FILTER_ICON) { self.presentationMode.wrappedValue.dismiss() }
                        button1Method: { self.showOptionsSheet = true }
                        button2Method: { self.showFilterSheet = true }
                        .actionSheet(isPresented: $showFilterSheet) {
                            ActionSheet(title: Text("Выберите фильтр"), buttons: [
                                    .default(Text("За всё время")) { filter = .all },
                                    .default(Text("Последние 7 дней")) { filter = .week },
                                    .default(Text("Последние 30 дней")) { filter = .month },
                                    .cancel()
                            ])
                        }
                    ExpenseMainView(filter: filter)
                        .actionSheet(isPresented: $showOptionsSheet) {
                            ActionSheet(title: Text("Выберите опцию"), buttons: [
                                    .default(Text("Настройки")) { self.displaySettings = true },
                                    .cancel()
                            ])
                        }
                    Spacer()
    ...
    var body: some View {
        ...
        ScrollView(showsIndicators: false) {
            ...
            if fetchRequest.wrappedValue.isEmpty {
                LottieView(animType: .kubiks_cube).frame(width: 300, height: 300)
                VStack {
                    TextView(text: "Операции ещё нет!", type: .h6).foregroundColor(Color.text_primary_color)
                    TextView(text: "Добавьте её, и она появится здесь (⌒‿⌒)", type: .body_1).foregroundColor(Color.text_secondary_color).padding(.top, 2)
                }.padding(.horizontal)
            } else {
                VStack(spacing: 16) {
                    TextView(text: "Общий баланс", type: .overline).foregroundColor(Color.init(hex: "828282")).padding(.top, 30)
                ...
                HStack {
                    TextView(text: "Недавние операции", type: .subtitle_1).foregroundColor(Color.text_primary_color)
                    Spacer()
                }.padding(4)
...
struct ExpenseModelView: View {
    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            HStack{
                TextView(text: isIncome ? "Доход" : "Расход", type: .overline).foregroundColor(Color.init(hex: "828282"))
                Spacer()
        ...
        HStack {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                HStack {
                    ...
                    Spacer()
                    TextView(text: getDateFormatter(date: expenseObj.occuredOn, format: "dd MMM, yyyy"), type: .body_2).foregroundColor(Color.text_primary_color)
                }
            }.padding(.leading, 4)
